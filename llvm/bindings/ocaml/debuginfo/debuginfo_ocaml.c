...
CAMLprim value llvm_ditype_get_size_in_bits(LLVMMetadataRef DType) {
  uint64_t Size = LLVMDITypeGetSizeInBits(DType);
  return Val_int((int)Size);
}

CAMLprim value llvm_ditype_get_offset_in_bits(LLVMMetadataRef DType) {
  uint64_t Size = LLVMDITypeGetOffsetInBits(DType);
  return Val_int((int)Size);
}

CAMLprim value llvm_ditype_get_align_in_bits(LLVMMetadataRef DType) {
  uint32_t Size = LLVMDITypeGetAlignInBits(DType);
  return Val_int(Size);
}

CAMLprim value llvm_ditype_get_line(LLVMMetadataRef DType) {
  unsigned Line = LLVMDITypeGetLine(DType);
  return Val_int(Line);
}

CAMLprim value llvm_ditype_get_flags(LLVMMetadataRef DType) {
